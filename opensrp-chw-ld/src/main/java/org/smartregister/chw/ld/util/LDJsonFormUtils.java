...
import static org.smartregister.chw.ld.util.Constants.ENCOUNTER_TYPE;
import static org.smartregister.chw.ld.util.Constants.STEP_ONE;
import static org.smartregister.chw.ld.util.Constants.STEP_TWO;

public class LDJsonFormUtils extends org.smartregister.util.JsonFormUtils {
    public static final String METADATA = "metadata";

    public static Triple<Boolean, JSONObject, JSONArray> validateParameters(String jsonString) {

        JSONObject jsonForm = toJSONObject(jsonString);
        JSONArray fields = ldFormFields(jsonForm);
    ...
    public static JSONArray ldFormFields(JSONObject jsonForm) {
        try {
    ...
    public static Event processJsonForm(AllSharedPreferences allSharedPreferences, String
        ...
        if (Constants.EVENT_TYPE.LD_CONFIRMATION.equals(encounter_type)) {
            encounter_type = Constants.TABLES.LD_CONFIRMATION;
        } else if (Constants.EVENT_TYPE.LD_FOLLOW_UP_VISIT.equals(encounter_type)) {
            encounter_type = Constants.TABLES.LD_FOLLOW_UP;
        }
    ...
    protected static FormTag formTag(AllSharedPreferences allSharedPreferences) {
        ...
        formTag.appVersion = LDLibrary.getInstance().getApplicationVersion();
        formTag.databaseVersion = LDLibrary.getInstance().getDatabaseVersion();
        return formTag;
    }
    ...
    public static void tagEvent(AllSharedPreferences allSharedPreferences, Event event) {
        ...
        event.setClientApplicationVersion(LDLibrary.getInstance().getApplicationVersion());
        event.setClientDatabaseVersion(LDLibrary.getInstance().getDatabaseVersion());
    }
    ...
    public static JSONObject getFormAsJson(String formName) throws Exception {
        return FormUtils.getInstance(LDLibrary.getInstance().context().applicationContext()).getFormJson(formName);
    }
