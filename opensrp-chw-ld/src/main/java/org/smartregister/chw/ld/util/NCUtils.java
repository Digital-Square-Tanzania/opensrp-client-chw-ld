...
import static org.smartregister.chw.ld.util.JsonFormUtils.cleanString;
...
public class NCUtils {
    ...
    public static org.smartregister.Context context() {
        return LDLibrary.getInstance().context();
    }
    ...
    public static SimpleDateFormat getSourceDateFormat() {
        return new SimpleDateFormat(LDLibrary.getInstance().getSourceDateFormat(), Locale.getDefault());
    }

    public static SimpleDateFormat getSaveDateFormat() {
        return new SimpleDateFormat(LDLibrary.getInstance().getSaveDateFormat(), Locale.getDefault());
    }
    ...
    public static Visit eventToVisit(Event event, String visitID) throws JSONException {
        ...
        if (eventDetails != null)
            visit.setVisitGroup(eventDetails.get(Constants.LD_VISIT_GROUP));
    ...
    public static void processHomeVisit(EventClient baseEvent, SQLiteDatabase database, String parentEventType) {
        try {
            Visit visit = LDLibrary.getInstance().visitRepository().getVisitByFormSubmissionID(baseEvent.getEvent().getFormSubmissionId());
            if (visit == null) {
                ...
                if (StringUtils.isNotBlank(parentEventType) && !parentEventType.equalsIgnoreCase(visit.getVisitType())) {
                    String parentVisitID = LDLibrary.getInstance().visitRepository().getParentVisitEventID(visit.getBaseEntityId(), parentEventType, visit.getDate());
                    visit.setParentVisitID(parentVisitID);
                }

                if (database != null) {
                    LDLibrary.getInstance().visitRepository().addVisit(visit, database);
                } else {
                    LDLibrary.getInstance().visitRepository().addVisit(visit);
                }
                    ...
                    for (Map.Entry<String, List<VisitDetail>> entry : visit.getVisitDetails().entrySet()) {
                        if (entry.getValue() != null) {
                            for (VisitDetail detail : entry.getValue()) {
                                if (database != null) {
                                    LDLibrary.getInstance().visitDetailsRepository().addVisitDetails(detail, database);
                                } else {
                                    LDLibrary.getInstance().visitDetailsRepository().addVisitDetails(detail);
                                }
                            }
                        }
                    }
