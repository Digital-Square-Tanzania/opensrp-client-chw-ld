...
import org.smartregister.chw.ld.listener.BaseLDCallWidgetDialogListener;
import org.smartregister.ld.R;

import static android.view.View.GONE;
import static org.smartregister.util.Utils.getName;

public class BaseLDCallDialogFragment extends DialogFragment implements BaseLDCallDialogContract.View {

    public static final String DIALOG_TAG = "BaseLDCallDialogFragment_DIALOG_TAG";
    private static MemberObject MEMBER_OBJECT;
    private View.OnClickListener listener = null;

    public static BaseLDCallDialogFragment launchDialog(Activity activity, MemberObject MO) {
        BaseLDCallDialogFragment dialogFragment = BaseLDCallDialogFragment.newInstance();
    ...
    public static BaseLDCallDialogFragment newInstance() {
        return new BaseLDCallDialogFragment();
    }
    ...
    private void setCallTitle(ViewGroup rootView, int viewId, final String message) {
        ...
        if (MEMBER_OBJECT.getBaseEntityId().equals(MEMBER_OBJECT.getFamilyHead())) {
            ...
            callTitle.setText(String.format("%s %s", message, getResources().getString(R.string.call_ld_client)));
        }
    }
    ...
    private void initUI(ViewGroup rootView) {
        if (StringUtils.isNotBlank(MEMBER_OBJECT.getPhoneNumber())) {
            ...
            if (StringUtils.isNotBlank(MEMBER_OBJECT.getFamilyHead())) {
                TextView familyHeadName = rootView.findViewById(R.id.ld_call_head_name);
                familyHeadName.setText(MEMBER_OBJECT.getFamilyHeadName());
                TextView clientCallHeadPhone = rootView.findViewById(R.id.ld_call_head_phone);
                ...
                rootView.findViewById(R.id.ld_layout_family_head).setVisibility(GONE);
            }

            if (!MEMBER_OBJECT.getBaseEntityId().equals(MEMBER_OBJECT.getFamilyHead())) {
                //just a member
                TextView ldClientNameTextView = rootView.findViewById(R.id.call_ld_client_name);
                ldClientNameTextView.setText(String.format("%s %s %s", MEMBER_OBJECT.getFirstName(), MEMBER_OBJECT.getMiddleName(), MEMBER_OBJECT.getLastName()));

                setCallTitle(rootView, R.id.call_ld_client_title, "");
                TextView callLDClientPhone = rootView.findViewById(R.id.call_ld_client_phone);
                callLDClientPhone.setTag(MEMBER_OBJECT.getPhoneNumber());
                callLDClientPhone.setText(getName(getCurrentContext().getString(R.string.call), MEMBER_OBJECT.getPhoneNumber()));
                callLDClientPhone.setOnClickListener(listener);
            } else {
                rootView.findViewById(R.id.layout_ld_client).setVisibility(GONE);
            }
        }

        rootView.findViewById(R.id.ld_call_close).setOnClickListener(listener);
    }
    ...
    @Override
    public void setPendingCallRequest(BaseLDCallDialogContract.Dialer dialer) {
